...
# ------------------------------------------------------------
# MAIN (partial -> paiement -> full) — place this block to replace the old MAIN
# ------------------------------------------------------------
...
# Upload handling: store uploaded bytes in session_state (so we can reuse after payment)
if uploaded is not None:
    st.success("PDF reçu ✅")
    # store raw bytes so we can reuse after redirect/payment
    st.session_state["uploaded_buffer"] = uploaded.getvalue()
    st.session_state["uploaded_name"] = uploaded.name

    # Run partial analysis now (no payment required)
    ...
    # store partial results in session_state
    ...
    # validate the document (is it a payslip? pages rules?)
    ...
    if not ok_doc:
        ...
        # Do NOT show payment button
        st.stop()

    # if OK, record and show preview + payment option
...
# If we already had an uploaded file in session_state (e.g. page refresh after payment redirect),
# show partial results and controls
elif st.session_state.get("uploaded_buffer"):
    st.success("Bulletin chargé depuis la session")
    # show partial results if present
...
# Show payment button only if partial_ok
if st.session_state.get("partial_ok"):
    ...
    # If we don't have partial_ok and no uploaded file, prompt to upload
    if not st.session_state.get("uploaded_buffer"):
        st.info("Dépose ton bulletin pour lancer l'analyse partielle (gratuit).")
    else:
        # uploaded exists but partial not OK: message already shown earlier; re-iterate
        st.info("Le document n'est pas exploitable automatiquement. Corrige le PDF et réessaie.")

# AFTER PAYMENT: user comes back with session_id in URL -> allow launching full analysis
session_id = _get_query_param("session_id") or _get_query_param("checkout_session_id")
payment_ok, payment_reason = is_payment_ok()

if session_id:
    if payment_ok:
        # payment done — but do not auto-consume: require explicit user click to start full analysis
        ...
        if credit_is_consumed(session_id):
            ...
            if st.button("Lancer l'analyse complète (après paiement)", type="primary"):
                ...
                # run_full_analysis already shows download button and UI
    else:
        # payment not ok / missing secret or not shipped yet
